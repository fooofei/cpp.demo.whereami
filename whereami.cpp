...
static
int _get_module_fullpath(HMODULE m, std::wstring * out)
{
    if (!out) return ERROR_FAIL;
    out->clear();
...
{
    HMODULE m = NULL;
    BOOL b = GetModuleHandleEx(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS | GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT
        , (LPCTSTR)get_library_fullpath, &m);
    if (!b) return ERROR_FAIL;
    if (!m) return ERROR_FAIL;
...
static
int get_executable_fullpath_utf8(std::string * out)
{
    ...
    if (!out) return ERROR_FAIL;
    out->clear(); out->assign(ret);
    return ERROR_NOERROR;
}

/* string is utf-8 bytes */
static
int get_library_fullpath_utf8(std::string * out)
{
    FILE * fmaps = NULL;
    ...
    int retval_temp;
    uint64_t  addr0 = (uint64_t)(uintptr_t)(void*)""; (void)addr0;
    uint64_t  addr1 = (uint64_t)(uintptr_t)(void*)get_library_fullpath_utf8; (void)addr1;
...
int get_executable_fullpath_utf8(std::string * out)
{
    ...
    if (0 != retval_temp) return ERROR_FAIL;
...
int get_library_fullpath(std::wstring * out)
{
    ...
    if (err != ERROR_NOERROR) return err;
    // utf-8 -> wstring
